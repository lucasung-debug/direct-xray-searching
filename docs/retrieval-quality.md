...
화면의 `심층 검색으로 더 찾기`는 이 통제 실험을 실제 후보 풀 확장 기능으로 연결합니다. 초기 검색을 대체하거나 같은 query를 반복하지 않고, 동일 조건에서 레퍼런스 직무 유형을 일반화한 별도 expansion facet 5개를 `advanced`로 한 번씩 실행합니다. 서버 중복 방지 서명은 초기·심층 라운드를 구분하고, 브라우저는 URL 기준으로 두 응답을 병합합니다. 같은 프로필이 다시 발견되면 새 평가가 기존 자동 카드를 갱신하되 초기 역할어, 심층 facet, 원문 링크는 합집합으로 남깁니다. 사람이 직접 검증한 카드의 이름·점수·요약은 계속 보존합니다.
...
- 심층 라운드의 5개 expansion facet은 초기 10개 query와 문자열이 겹치지 않고, 레퍼런스의 최고책임자 변형·부문/센터장·전직 CPO/CISO+cloud·선임심사/국제표준·Privacy 전문 커뮤니티 유형을 별도 표현으로 탐색함
...
| Tavily credits | 라운드별 10 이하 · 초기+심층 합계 20 이하 |
