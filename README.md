...
- 서비스: [Live demo](https://dreamus-cpo-direct-sourcing.masterddinng.chatgpt.site)
...
검색 중에는 공급자용 긴 실행어를 노출하지 않고 진행 단계만 보여줍니다. 키워드 5개 기준으로 다음 세 종류의 검색을 합칩니다.

- 정확한 역할어 검색 5회
- 유사 직함 검색 5회
- 프리셋이 정의한 전문근거 검색 5회

초기 검색은 키워드마다 위 3개 검색면을 Tavily `basic`으로 동시에 실행합니다. 키워드 5개 기준 총 15회, 최대 15 credits를 사용하며, 결과는 URL 기준으로 중복 제거해 하나의 후보 풀로 합칩니다. 수동 Google·Bing 분기 버튼은 제공하지 않습니다.
...
- Tavily의 공개 웹 색인에 노출되지 않은 프로필은 발견할 수 없습니다.
...
- Tavily 직함·유사직함·전문근거 병렬 검색 및 URL 기준 후보 풀 병합
...
- [ ] Live demo에서 첫 검색과 심층 검색이 각각 정상 완료되는가
